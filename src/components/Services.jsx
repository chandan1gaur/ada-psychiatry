...
const Services = () => {
  ...
  return (
 <Section id="services" className="bg-cream-50 max-w-7xl mx-auto px-4">
  <div className="text-center mb-12 sm:mb-16">
    <h2 className="font-playfair-display text-[28px] sm:text-[36px] md:text-[42px] font-semibold text-[#3b2f2a]">
      Our Services
    </h2>
  </div>
  ...
  <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-8 xl:gap-6 justify-items-center">
    {services.map((service, index) => (
      <div key={index} className="text-center flex flex-col items-center gap-4">
        <div className="w-[100px] h-[100px] rounded-full bg-[#F7EED9] flex items-center justify-center">
          <img src={service.icon} alt={service.alt} className="w-8 h-8" />
        </div>
        <h3 className="font-wulkan-display font-bold text-[20px] leading-[28px] text-[#1B1B1B] text-center max-w-[150px]">
          {service.title}
        </h3>
      </div>
    ))}
  ...
  <div className="mt-12 sm:mt-14 flex justify-center">
    <button className="font-work-sans font-medium text-[14px] sm:text-[15px] text-black rounded-full hover:opacity-90 transition-opacity flex items-center justify-center gap-2 w-[210px] sm:w-[240px] h-[44px] sm:h-[50px] bg-gradient-to-r from-[#C18C2C] via-[#FCF38A] to-[#C18C2C]">
      View our services
